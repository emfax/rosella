...
import { type SlotOccupiedController } from '../../control/slot.js';
...
export class SubmenuController implements ReactiveController {
  ...
  private popupRef: Ref<Popover> = createRef();
  private enableSubmenuTimer = -1;
  private isConnected = false;
  private isPopupConnected = false;
  private skidding = 0;
  private readonly slotOccupiedController: SlotOccupiedController;
  private readonly submenuOpenDelay = 100;

  constructor(host: ReactiveControllerHost & MenuItem, slotOccupiedController: SlotOccupiedController) {
    (this.host = host).addController(this);

    this.slotOccupiedController = slotOccupiedController;
  }

  hostConnected() {
    if (this.slotOccupiedController.test('submenu') && !this.host.disabled) {
      this.addListeners();
    }
  }
  ...
  private handleMouseOver = () => {
    if (this.slotOccupiedController.test('submenu')) {
      this.enableSubmenu();
    }
  };
